...
Prereg: `REPORT/REG/R5/R5_D0_PREFLIGHT_PREREG.md`  
...
That is a **ranking crossover**, hence **oracle** decision relevance
(an optimal user of \(X\) would switch at \(\lambda=0\)). It is not yet
realized \(\mathrm{VoI}_{\Pi}\). A planner that cannot see \(X\) and
sees identical \(h^{S}\) must pick **one** action for all \(\lambda\);
oracle does not. Whether the frozen world-model planner reduces
**executed regret** is D0 proper. Do not retune \(\mathcal A\) or \(J\)
to make \(a_{\mathrm{agg}}\) win at high \(\lambda\).
