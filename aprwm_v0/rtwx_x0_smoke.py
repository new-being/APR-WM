...
PREREG_PATH = "REPORT/REG/RTWX/robot_dynamics/RTWX0_SMOKE_PREREG.md"
...
def _patch_curobo_planner(repo: Path) -> bool:
    ...
    class _StubCuroboPlanner:
        ...
        def plan_grippers(self, now_val, target_val) -> dict:
            num_step = 200
...
def extract_oracle_state(env: Any) -> dict[str, Any]:
...
def run_rtwx_x0_smoke(output: str | Path, config: RTWX0SmokeConfig | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        env = None
        last_asset_err: Exception | None = None
        used_seed = cfg.seed
        for seed in range(cfg.seed, cfg.seed + max(1, cfg.seed_attempts)):
            ...
            try:
                ...
                try:
                    ...
                    pass
                env = None
        if last_asset_err is not None:
            raise last_asset_err
